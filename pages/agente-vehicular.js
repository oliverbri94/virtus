...
export default function PlanificadorVehicular() {
  ...
  // Libro Mayor Filters
  const [listFilterType, setListFilterType] = useState('todos'); // 'todos', 'ingreso', 'gasto', 'kilometraje'
  ...
  const categoriasKm = ['trabajo', 'personal'];
  ...
  // Indicadores Operativos (KMs)
  const kmsTrabajo = filteredTransactions.filter(t => t.tipo === 'kilometraje' && t.categoria === 'trabajo').reduce((acc, t) => acc + t.monto, 0);
  ...
  // Para Area + Line Chart (Evolución Diaria Diaria)
  const trendData = useMemo(() => {
    const grouped = filteredTransactions.reduce((acc, curr) => {
      const date = curr.fecha;
      if (!acc[date]) acc[date] = { date, ingreso: 0, gasto: 0, neto: 0, kms: 0 };
      if (curr.tipo === 'ingreso') {
        acc[date].ingreso += curr.monto;
        acc[date].neto += curr.monto;
      } else if (curr.tipo === 'gasto') {
        acc[date].gasto += curr.monto;
        acc[date].neto -= curr.monto;
      } else if (curr.tipo === 'kilometraje') {
        acc[date].kms += curr.monto;
      }
      return acc;
    }, {});
  ...
  // Limpiar categoría al cambiar tipo de filtro en Libro Mayor
  useEffect(() => {
    ...
    <div className="min-h-screen bg-[#040814] text-slate-100 font-sans selection:bg-indigo-500/30 p-4 sm:p-6 lg:p-8">
      ...
      <div className="max-w-[1400px] mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-slate-900/60 border border-slate-800 p-4 sm:p-5 rounded-2xl sm:rounded-3xl shadow-lg hover:shadow-emerald-900/20 hover:border-emerald-500/30 transition-all group overflow-hidden relative">
          ...
          <div className="bg-slate-900/60 border border-slate-800 p-4 sm:p-5 rounded-2xl sm:rounded-3xl shadow-lg hover:shadow-rose-900/20 hover:border-rose-500/30 transition-all group overflow-hidden relative">
          ...
          <div className="bg-slate-900/60 border border-slate-800 p-4 sm:p-5 rounded-2xl sm:rounded-3xl shadow-lg hover:shadow-cyan-900/20 hover:border-cyan-500/30 transition-all group overflow-hidden relative">
          ...
          <div className={`p-4 sm:p-5 rounded-2xl sm:rounded-3xl border shadow-lg group overflow-hidden relative ${gananciaNeta >= 0 ? 'bg-gradient-to-b from-slate-900 to-emerald-950/30 border-emerald-500/20' : 'bg-gradient-to-b from-slate-900 to-rose-950/30 border-rose-500/20'}`}>
            <div className="flex justify-between items-start mb-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl shadow flex items-center justify-between">
             <div className="flex items-center gap-3">
               ...
               <div>
                 <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Metraje Total</p>
                 <p className="text-xl font-bold text-white leading-tight">{totalKms.toFixed(1)} <span className="text-xs text-slate-500">km</span></p>
               </div>
             </div>
             <div className="text-right flex flex-col items-end gap-1">
               <div className="flex items-center gap-1.5 text-[10px] text-slate-400 font-bold bg-slate-950/50 px-2 py-0.5 rounded-full border border-slate-800">
                 <span className="w-1.5 h-1.5 rounded-full bg-indigo-400"></span> Trabajo: {kmsTrabajo.toFixed(0)}
               </div>
               <div className="flex items-center gap-1.5 text-[10px] text-slate-400 font-bold bg-slate-950/50 px-2 py-0.5 rounded-full border border-slate-800">
                 <span className="w-1.5 h-1.5 rounded-full bg-slate-500"></span> Personal: {kmsPersonal.toFixed(0)}
               </div>
             </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          {/* Main Trend Chart */}
          <div className="lg:col-span-2 bg-slate-900/60 border border-slate-800 p-4 sm:p-6 rounded-2xl sm:rounded-3xl shadow-xl">
            <div className="flex items-center gap-2 mb-6">
              <LineChartIcon className="w-5 h-5 text-indigo-400" />
              <h3 className="text-lg font-bold text-slate-100">Tendencia Financiera & Rodamiento</h3>
            </div>
              ...
              {trendData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={trendData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    ...
                    />
                    <YAxis yAxisId="left" stroke="#475569" fontSize={11} tickLine={false} axisLine={false} tickFormatter={(value) => `$${value}`} />
                    <YAxis yAxisId="right" orientation="right" stroke="#eab308" fontSize={11} tickLine={false} axisLine={false} tickFormatter={(value) => `${value}km`} />
                    <RechartsTooltip content={<CustomTooltipArea />} />
                    <Legend iconType="circle" wrapperStyle={{ fontSize: '12px' }} />
                    <Area yAxisId="left" type="monotone" name="Ingresos" dataKey="ingreso" stroke="#10b981" strokeWidth={3} fillOpacity={1} fill="url(#colorIngreso)" />
                    <Area yAxisId="left" type="monotone" name="Gastos" dataKey="gasto" stroke="#f43f5e" strokeWidth={3} fillOpacity={1} fill="url(#colorGasto)" />
                    <Line yAxisId="right" type="monotone" name="Kilómetros" dataKey="kms" stroke="#eab308" strokeWidth={2} dot={{ r: 3, fill: '#eab308' }} activeDot={{ r: 5 }} />
                  </ComposedChart>
                </ResponsiveContainer>
              ) : (
          ...
          <div className="bg-slate-900/60 border border-slate-800 p-4 sm:p-6 rounded-2xl sm:rounded-3xl shadow-xl flex flex-col">
              ...
              <form onSubmit={handleAddTransaction} className="space-y-4">
                ...
                <div className="p-1 bg-slate-950/80 rounded-xl flex gap-1 border border-slate-800/80 shadow-inner">
                  <button
                    ...
                    className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition-all ${tipo === 'gasto' ? 'bg-rose-500/20 text-rose-400 shadow-sm' : 'text-slate-500 hover:bg-slate-800 hover:text-slate-300'}`}
                  >
                    📉 Gasto
                  </button>
                  <button
                    ...
                    className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition-all ${tipo === 'ingreso' ? 'bg-emerald-500/20 text-emerald-400 shadow-sm' : 'text-slate-500 hover:bg-slate-800 hover:text-slate-300'}`}
                  >
                    📈 Ingreso
                  </button>
                  <button
                    type="button"
                    onClick={() => setTipo('kilometraje')}
                    className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition-all ${tipo === 'kilometraje' ? 'bg-amber-500/20 text-amber-400 shadow-sm' : 'text-slate-500 hover:bg-slate-800 hover:text-slate-300'}`}
                  >
                    🚗 KMs
                  </button>
                </div>
                ...
                <div>
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
                    {tipo === 'kilometraje' ? <MapPin className="w-3 h-3" /> : <DollarSign className="w-3 h-3" />} 
                    {tipo === 'kilometraje' ? 'Distancia (KM)' : 'Monto'}
                  </label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950/50 border border-slate-800 text-white rounded-xl py-2.5 pl-10 pr-4 focus:outline-none focus:ring-1 focus:ring-indigo-500/50 transition-all font-semibold text-base"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1.5 flex items-center gap-1.5">
                      <Tag className="w-3 h-3" /> {tipo === 'kilometraje' ? 'Uso' : 'Categoría'}
                    </label>
                    <select
                      value={categoria}
                    ...
                    >
                      {(tipo === 'gasto' ? categoriasGasto : tipo === 'ingreso' ? categoriasIngreso : categoriasKm).map(c => (
                ...
                <button
                  ...
                  className={`w-full py-3 mt-1 rounded-xl font-bold flex items-center justify-center gap-2 transition-transform active:scale-[0.98] text-sm shadow-lg
                    ${tipo === 'ingreso' ? 'bg-emerald-600 hover:bg-emerald-500 text-white shadow-emerald-900/30' : (tipo === 'gasto' ? 'bg-rose-600 hover:bg-rose-500 text-white shadow-rose-900/30' : 'bg-amber-600 hover:bg-amber-500 text-white shadow-amber-900/30')}`}
                >
                  Confirmar Registro
                </button>
              </form>
              ...
              <div className="p-4 sm:p-5 border-b border-slate-800/60 flex flex-col xl:flex-row xl:items-center justify-between gap-4">
                ...
                <div className="flex flex-wrap gap-2 items-center">
                  ...
                  >
                    ...
                    <option value="kilometraje">🚗 KMs</option>
                  </select>
                  ...
                  >
                    ...
                    {listFilterType === 'kilometraje' && categoriasKm.map(c => <option key={c} value={c} className="capitalize">{c}</option>)}
                    {listFilterType === 'todos' && [...categoriasGasto, ...categoriasIngreso, ...categoriasKm].map(c => <option key={c} value={c} className="capitalize">{c}</option>)}
                  </select>
                    ...
                    {listTransactions.map((tx) => (
                      <div key={tx.id} className="flex items-center justify-between p-3.5 bg-slate-950/40 border border-slate-800/50 rounded-2xl hover:bg-slate-800/80 transition-colors group">
                        ...
                        <div className="flex items-center gap-3.5">
                          <div className={`p-2.5 rounded-xl border ${tx.tipo === 'ingreso' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : (tx.tipo === 'gasto' ? 'bg-rose-500/10 border-rose-500/20 text-rose-400' : 'bg-amber-500/10 border-amber-500/20 text-amber-400')}`}>
                            {tx.tipo === 'ingreso' ? <TrendingUp className="w-4 h-4" /> : (tx.tipo === 'gasto' ? <TrendingDown className="w-4 h-4" /> : <MapPin className="w-4 h-4" />)}
                          </div>
                        ...
                        <div className="flex items-center gap-3">
                          <span className={`font-black text-sm md:text-base ${tx.tipo === 'ingreso' ? 'text-emerald-400' : (tx.tipo === 'gasto' ? 'text-rose-400' : 'text-amber-400')}`}>
                            {tx.tipo === 'kilometraje' ? `${tx.monto} km` : `${tx.tipo === 'ingreso' ? '+' : '-'}$${tx.monto.toFixed(2)}`}
                          </span>
                          <button 
